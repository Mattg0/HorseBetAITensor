...
def insert_data_into_sqlite(sqlite_db, data, columns):
    ...
    insert_query = f"INSERT OR IGNORE INTO Entraineur ({', '.join(columns)}) VALUES ({placeholders})"
...
def main():
    ...
    mysql_query = "SELECT idEntraineur ,entraineur as nom FROM cachedate;"  # Change this to your MySQL query

    # SQLite database file
    sqlite_db = "/Users/Mattg0/Docs/HorseBetAI/data/hippique.db"
    ...
    if data is not None and columns is not None:
        ...
        identraineur_index = columns.index('idEntraineur')  # Get the index of the idche column
        unique_data = remove_duplicates(data, identraineur_index)
